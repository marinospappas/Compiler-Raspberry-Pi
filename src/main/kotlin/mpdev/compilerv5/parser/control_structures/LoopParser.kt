...
class LoopParser(val context: CompilerContext) {
    ...
    fun parseBreak(label: String) {
        scanner.match()
        if (label == "")
            abort("(${this.javaClass.simpleName}) line ${scanner.currentToken().lineNumber}: no loop to break of")
        code.jump(label)
    }
    ...
    fun parseContinue(label: String) {
        scanner.match()
        if (label == "")
            abort("(${this.javaClass.simpleName}) line ${scanner.currentToken().lineNumber}: no loop to continue")
        code.jump(label)
    }
}
